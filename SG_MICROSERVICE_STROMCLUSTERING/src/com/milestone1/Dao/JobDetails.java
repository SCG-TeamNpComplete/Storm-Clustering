...
public class JobDetails {
	...
	public List<JobDetailsBean> getJobDetails() throws Exception {
		...
		for (String jobs : jobsList) {
			...
			if (bean.getTasks().get(0).getAssignedTask().slaveHost == "sga-mesos-slave-2") {
				host = "54.215.219.32";
			} else {
				host = "52.53.179.0";
			}

			String gif1 = host + ":1338/file/" + bean.getTasks().get(0).getAssignedTask().taskId
					+ "/wrfoutput/Surface_multi.gif";
			String gif2 = host + ":1338/file/" + bean.getTasks().get(0).getAssignedTask().taskId
					+ "/wrfoutput/Precip_total.gif";
			jdb.add(new JobDetailsBean(jobs, bean.getTasks().get(0).assignedTask.task.ramMb,
					bean.getTasks().get(0).assignedTask.task.numCpus, bean.getTasks().get(0).assignedTask.task.diskMb,
					gif1, gif2));
